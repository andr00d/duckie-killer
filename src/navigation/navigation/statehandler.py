...
class Statehandler(Node):
    ...
    # function expects the first message to be the gesture if one is sent. 
    def _objects_callback(self, msgs): 
        if len(msgs.objects) > 0:    
            ...
            if obj_msg.gesture == STOP and self.curr_state != STOP:
                self.get_logger().info(f'{obj_msg.gesture}    {self.curr_state}')
                obj_msg = Object(); 
                ...
                if obj_msg.gesture == SURV and self.curr_state != SURV:
                    self.curr_state = SURV
                    self.get_logger().info("Transitioned to SURVEILLANCE mode")
                elif obj_msg.gesture == GUARD and self.curr_state != GUARD:
                    self.curr_state = GUARD
                    self.get_logger().info("Transitioned to GUARD mode")
                elif obj_msg.gesture == HOME and self.curr_state != HOME:
